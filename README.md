...
```powershell
.\scripts\build.ps1 -Version 1.6.2
```

指定目标平台：

```powershell
.\scripts\build.ps1 -Version 1.6.2 -GoOS windows -GoArch amd64
```
...
插件当前版本为 **1.6.2**。GitHub Actions 支持手动输入 `vMAJOR.MINOR.PATCH` 格式的标签，例如 `v1.6.2`，并自动构建各平台压缩包和校验文件。
