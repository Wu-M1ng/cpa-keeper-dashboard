...
```powershell
.\scripts\build.ps1 -Version 1.2.5
```
...
```powershell
.\scripts\build.ps1 -Version 1.2.5 -GoOS windows -GoArch amd64
```
...
也可在 GitHub Actions 手动运行 `release` 工作流并输入版本标签，例如 `v1.2.5`。工作流会检查远端 tag：不存在时在当前提交创建并推送，存在时直接复用；对应 GitHub Release 已存在时会更新同名发布资产。
